  ...
  return (
    <div className="w-80 h-screen bg-background border-r border-border flex flex-col p-6 sticky top-0">
      <div className="flex items-center gap-4 px-2 py-8 mb-10">
        ...
        <div>
          <h1 className="font-bold text-2xl tracking-tighter text-white">Аналітика відділу</h1>
        </div>
      </div>
      ...
      <nav className="flex-1 space-y-2">
        {navItems.map((item) => (
          ...
          >
            <item.icon
              size={22}
              className={`${activeTab === item.id ? 'text-primary' : 'text-secondary group-hover:text-white'} transition-colors`}
            />
