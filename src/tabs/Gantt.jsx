...
// 3D Volumetric color palette for Gantt bars (Convex, glossy highlights, smooth gradients)
...
const Gantt = () => {
  ...
  const getTaskTimeframe = (project) => {
    ...
    if (!end) {
      const durationDays = Math.max(1, Math.ceil((project.points || 1) / 8));
      end = new Date(start);
  ...
  // Grouped by Performer structure
  const groupedData = useMemo(() => {
    if (!groupByEmployee) {
      return [{ groupName: null, items: filteredProjects }];
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-gray-800 flex items-center gap-3">
            ...
            Діаграма Ганта та план задач
          </h2>
          <p className="text-gray-500 font-medium text-sm mt-1">
            План виконання задач для відділу {activeDepartment === 'construction' ? 'Конструювання' : 'Проєктування'}
          </p>
        </div>

        {/* Zoom & Navigation buttons (Convex neu-btn / neu-flat) */}
      ...
      {/* Filters & Sorting Toolbar (Convex neumorphic cards) */}
      <div className="neu-flat p-4 rounded-2xl flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className={`neu-btn px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all ${
            groupByEmployee
        ...
        >
          ...
          <span>{groupByEmployee ? 'Групування: За виконавцями' : 'Групування: Список'}</span>
        </button>
        ...
        <div className="w-48">
          <select
            ...
            className="w-full neu-flat px-3.5 py-2 rounded-xl text-xs font-bold text-gray-700 outline-none bg-[#e0e5ec] cursor-pointer"
          >
            <option value="all">Усі активні задачі</option>
        ...
        <div className="text-xs font-extrabold text-gray-500 px-2 ml-auto">
          Активних задач: <span className="text-primary font-black">{filteredProjects.length}</span>
        </div>
      </div>

      {/* Gantt Table Viewport (Convex Container) */}
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[1100px]">
            ...
            <div className="flex border-b border-gray-300/80 pb-3 mb-4">
              {/* Left Column Headers (Interactive Sorting: Only Performer and Project) */}
              ...
              <div className="space-y-3 relative">
                ...
                {groupedData.map((group, gIdx) => (
                  <div key={gIdx} className="space-y-2">
                    {/* Performer Group Header (Convex Card) */}
                    {group.groupName && (
                      <div className="flex items-center justify-between neu-flat px-4 py-2 rounded-xl border border-white/60 shadow-sm">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-[10px] text-gray-500 font-medium">({group.items.length} задач у роботі)</span>
                        </div>
                      </div>
                    )}

                    {/* Task Rows inside Group (Convex Elevated Rows) */}
                    {group.items.map((project) => {
                      ...
                      return (
                        <div
                          key={project.id}
                          className="flex items-center neu-flat p-2.5 rounded-2xl hover:brightness-[1.02] hover:-translate-y-0.5 transition-all group cursor-pointer border border-white/40"
                          ...
                          <div className="w-[24rem] shrink-0 grid grid-cols-12 gap-2 pr-3 pl-1 items-center">
                            ...
                            <div className="col-span-7 min-w-0 pr-1">
                              ...
                              <p className="text-[10px] text-gray-400 font-mono">
                                {formatDateShort(tf.startDate)} ➔ {formatDateShort(tf.endDate)} ({tf.durationDays}д)
                              </p>
                            </div>
                          </div>

                          {/* Gantt Timeline Bar Track (Clean Convex Surface with 3D Bar) */}
                            ...
                            {isVisibleInView && (
                              ...
                              >
                                ...
                                <span className="text-[9px] font-mono opacity-95 shrink-0 whitespace-nowrap bg-black/25 px-1.5 py-0.5 rounded shadow-sm">
                                  {formatDateShort(tf.startDate)} - {formatDateShort(tf.endDate)}
                                </span>
                              </div>
                            )}
                          </div>
      ...
      {selectedTaskDetails && (() => {
        const tf = getTaskTimeframe(selectedTaskDetails);
            ...
            <div className="bg-[#e0e5ec] p-6 rounded-2xl neu-flat max-w-lg w-full border border-white/50 shadow-2xl space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="neu-flat p-3 rounded-xl">
                  ...
                  <p className="text-sm font-extrabold text-gray-800 truncate">
                    {selectedTaskDetails.assignedEmployee || 'Не призначено'}
                  </p>
                </div>
              </div>
              ...
              <div className="neu-flat p-3.5 rounded-xl text-xs space-y-1.5 text-gray-600 font-medium">
                <div className="flex justify-between">
                  <span>Обсяг складності (Поінти):</span>
                  <span className="font-bold text-gray-800">{selectedTaskDetails.points || 0} поінтів</span>
                </div>
      ...
      {/* Legend & Summary (Calendar markers) */}
      <div className="flex flex-wrap items-center justify-end gap-4 px-2 pt-1 text-xs font-bold text-gray-500">
